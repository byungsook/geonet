...
def conv2d(x, o_dim, data_format='NHWC', name=None, k=4, s=2, act=None, bias=True):
    if bias:
...
def conv2d_batch(x, o_dim, data_format='NHWC', name=None, k=4, s=2, act=None, no_bias=False):
    return slim.conv2d(x, o_dim, k, stride=s, activation_fn=act, scope=name, data_format=data_format)
...
def denorm_img(norm, data_format):
    ...
    return tf.cast(tf.clip_by_value(to_nhwc(norm*255, data_format), 0, 255), tf.uint8)
